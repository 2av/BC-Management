<?php
/**
 * Multi-Tenant BC Management System Configuration
 * This file extends the existing config.php with multi-tenant functionality
 */

// Include the original config
require_once 'config.php';

// Multi-tenant specific functions

// Super Admin Authentication Functions (only declare if not already defined)
    ...
    function getAllClients() {
        ...
        $stmt = $pdo->query("
            SELECT c.*, sa.full_name as created_by_name,
                   cs.total_groups, cs.total_members, cs.active_groups, cs.completed_groups
            FROM clients c
...
if (!function_exists('createClient')) {
    function createClient($data) {
        ...
        if ($result) {
            ...
            // Log the action
            if (function_exists('logAuditAction')) {
                logAuditAction($clientId, 'super_admin', $_SESSION['super_admin_id'], 'create_client', 'clients', $clientId);
            }
...
// Override existing functions to include client context
function getAllGroupsForClient($clientId = null) {
...
function getGroupByIdForClient($id, $clientId = null) {
    $clientId = $clientId ?? getCurrentClientId();
    if (!$clientId) return null;
