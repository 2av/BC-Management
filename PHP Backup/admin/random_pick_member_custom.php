...
$selectedMemberId = (int)($_POST['selected_member_id'] ?? 0);

if (!$groupId || !$monthNumber || !$selectedMemberId) {
    echo json_encode(['success' => false, 'message' => 'Group ID, Month Number, and Selected Member ID are required']);
    exit;
}
...
try {
    ...
    // Verify selected member exists and is in the group
    $stmt = $pdo->prepare("
        SELECT m.id, m.member_name, gm.member_number
        FROM group_members gm
        JOIN members m ON gm.member_id = m.id
        WHERE gm.group_id = ? AND m.id = ? AND gm.status = 'active'
    ");
    $stmt->execute([$groupId, $selectedMemberId]);
    $selectedMember = $stmt->fetch();
    
    if (!$selectedMember) {
        echo json_encode(['success' => false, 'message' => 'Selected member not found or not in this group']);
        exit;
    }
    ...
    if ($stmt->execute([$groupId, $monthNumber, $selectedMemberId, $pickedBy, $pickedByType])) {
        echo json_encode([
            'success' => true, 
